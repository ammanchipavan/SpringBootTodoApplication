package com.infor.todo.springboot.controller;
...
import com.infor.todo.springboot.service.TodoService;
import com.infor.todo.springboot.util.CustomErrorType;
...
public class RestApiController {
	...
	public ResponseEntity<?> getTodo(@PathVariable("id") long id) {
		...
		if (todo == null) {
			...
			return new ResponseEntity<CustomErrorType>(new CustomErrorType("Task with id " + id 
	...
	public ResponseEntity<?> createTodo(@RequestBody Todo todo, UriComponentsBuilder ucBuilder) {
		...
		if (todoService.isTodoExist(todo)) {
			...
			return new ResponseEntity<CustomErrorType>(new CustomErrorType("Unable to create. A Task with name " + 
	...
	public ResponseEntity<?> updateTodo(@PathVariable("id") long id, @RequestBody Todo todo) {
		...
		if (todoTask == null) {
			...
			return new ResponseEntity<CustomErrorType>(new CustomErrorType("Unable to upate. Task with id " + id + " not found."),
	...
	public ResponseEntity<?> deleteTodo(@PathVariable("id") long id) {
		...
		if (todo == null) {
			...
			return new ResponseEntity<CustomErrorType>(new CustomErrorType("Unable to delete. Task with id " + id + " not found."),
					HttpStatus.NOT_FOUND);
		}
